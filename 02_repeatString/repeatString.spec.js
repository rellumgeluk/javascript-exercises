...
describe('repeatString', () => {
  let randomTestNumber;

  beforeEach(() => {
    randomTestNumber = Math.ceil(Math.random() * 1000);
  });
  ...
  test('repeats the string a random amount of times', function () {
    ...
    const result = repeatString('hallo', randomTestNumber);
    if (randomTestNumber > 0) {
    expect(result.match(/hallo/g).length).toEqual(randomTestNumber);
    } else {
  ...
  test('hallo', () => {
    expect(repeatString('hallo', 13)).toEqual('hallohallohallohallohallohallohallohallohallohallohallohallohallo');
  });
});